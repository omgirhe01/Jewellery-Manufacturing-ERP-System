...
from app.models.all_models import Job, JobStageLog, Department, Customer, User, Design
...
class JobCreate(BaseModel):
    design_name: str
    design_id: Optional[int] = None   # Link to existing design from library
    customer_id: int
    metal_type: str
    target_weight: float
...
def create_job(data: JobCreate, db: Session = Depends(get_db),
    ...
    if data.design_id:
        design = db.query(Design).filter(Design.id == data.design_id).first()
        if not design:
    ...
    job = Job(job_code=job_code, barcode=barcode_val, barcode_image_b64=barcode_b64,
              design_name=design_name, design_id=data.design_id,
    ...
    for i, dept in enumerate(depts):
        stage = JobStageLog(job_id=job.id, department_id=dept.id, stage_name=dept.name,
...
def get_job(job_id: int, db: Session = Depends(get_db),
            current_user: User = Depends(get_current_user)):
    job = db.query(Job).options(joinedload(Job.customer), joinedload(Job.stage_logs)).filter(Job.id == job_id).first()
    if not job:
...
def get_by_barcode(barcode: str, db: Session = Depends(get_db),
                   current_user: User = Depends(get_current_user)):
    job = db.query(Job).options(joinedload(Job.customer), joinedload(Job.stage_logs)).filter(Job.barcode == barcode).first()
    if not job:
...
def advance_stage(job_id: int, weight_out: float = 0, notes: str = "",
    ...
    curr_stage = db.query(JobStageLog).filter(JobStageLog.job_id == job_id,
                                            JobStageLog.stage_name == job.current_stage).first()
    if curr_stage:
    ...
    next_stage = db.query(JobStageLog).filter(JobStageLog.job_id == job_id,
                                            JobStageLog.stage_name == next_name).first()
    if next_stage:
...
def list_designs_for_jobs(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Get all designs for dropdown in job creation"""
    designs = db.query(Design).order_by(Design.name).all()
    return [{
        "id": d.id,
        "design_code": d.design_code,
        "name": d.name,
        "description": d.description,
        "has_image": bool(d.image_path)
    } for d in designs]


@router.get("/{job_id}/design-image")
def get_job_design_image(job_id: int, db: Session = Depends(get_db),
                         current_user: User = Depends(get_current_user)):
    """Get design image for a job (from linked design library)"""
    job = db.query(Job).filter(Job.id == job_id).first()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    if not job.design_id:
        raise HTTPException(status_code=404, detail="No design linked to this job")
    import os, base64
    design = db.query(Design).filter(Design.id == job.design_id).first()
    if not design or not design.image_path or not os.path.exists(design.image_path):
